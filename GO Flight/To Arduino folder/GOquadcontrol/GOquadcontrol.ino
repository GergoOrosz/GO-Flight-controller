// GO Flight attitude control algorithm 1.0       Quadcopter flight system based on two Arduino micros
...
// Please note that this code has never been tried on a quadcopter, I am not a quadflyer, please if you decide to try this on a quad, provide feedback on the website.
...
#define Ailgain 1.8 // proportional gain
#define AilIgain 2.9 // integral gain, which determines the Ail position holding power of the system
...
#define RudPgain 1.8 // 3.5 //proportional gain of the tail
#define RudHHgain 2 // 32 // Heading-Hold (Avcs) gain of the tail
#define Rudstopgain 0.02 // 1.25 Rudder stop gain
...
#define Pitchprecomp 0 // Mixes a certain amount of Pitch to Tail                         These are not to be used on a quadcopter
#define Cyclicprecomp 0 // Mixes a certain amount of Cyclic to Tail
#define Eleprecomp 0 // Elevator precompensation, Mixes a bit of Elevator to Pitch

//  Feedforvard amount determines how much cyclic command is applied directly to the swashplate
#define Feedforward 0//note used on a quadcopter
...
// reverses the correction direction of controls. If does not work, change the cor sign in the AILin Elein Back section
...
double aggKpR=RudPgain*agrgain, aggKiR=RudHHgain*agrgain, aggKdR=Rudstopgain;
double consKpR=RudPgain, consKiR=RudHHgain, consKdR=Rudstopgain;
double Ailgap=0;
double Elegap=0;
double Rudgap=0;
double Wxbi = 0;
...
void setup()
{
  ...
  RudPID.SetOutputLimits(-400, 400);

  // attach servo objects, these will generate the correct
  // pulses for driving Electronic speed controllers, servos or other devices
  // designed to interface directly with RC Receivers 
  CRCArduinoFastServos::attach(SERVO_AIL,Frontmotor);
  CRCArduinoFastServos::attach(SERVO_ELE,Leftmotor);
  CRCArduinoFastServos::attach(SERVO_PIT,Rightmotor);
  CRCArduinoFastServos::attach(SERVO_RUD,Backmotor);
...
{ 
  ...
  Ailcormap = map(Ailcor,-400,400,-400+Ailtrim-Ailendpoint,400+Ailtrim+Ailendpoint);
  Elecormap = map(Elecor,-400,400,-400+Eletrim-Eleendpoint,400+Eletrim+Eleendpoint);
  Rudcormap = map(Rudcor,-400,400,-400+Rudtrim-Rudendpoint,400+Rudtrim+Rudendpoint);
  Throttlein = map(unPitchIn+Pittrim,1100 + Pittrim,1900+Pittrim,-400+Pittrim-Pitendpoint,400+Pittrim+Pitendpoint);
  
  //Pure control values for further mixes
...
//Right = Throttle - RollPID + YawPID
  ...
  Front = map(Throttlein +Elecormap - Rudcormap,-1200,1200,1000+Fronttrim,2000+Fronttrim);
  Left =map(Throttlein -Elecormap - Rudcormap,-1200,1200,1000+Lefttrim,2000+Lefttrim);
  Right = map(Throttlein + Ailcormap +Rudcormap,-1200,1200,1000+Righttrim,2000+Righttrim);
  Back = map(Throttlein - Ailcormap +Rudcormap,-1200,1200,1000+Backtrim,2000+Backtrim);
  ...
  if(bUpdateFlags & AIL_FLAG)
  {
    CRCArduinoFastServos::writeMicroseconds(SERVO_AIL,Front);
  }

  if(bUpdateFlags & ELE_FLAG)
  {
    CRCArduinoFastServos::writeMicroseconds(SERVO_ELE,Left);
  }

  if(bUpdateFlags & PIT_FLAG)
  {
   CRCArduinoFastServos::writeMicroseconds(SERVO_PIT,Right);
   }
   
    if(bUpdateFlags & RUD_FLAG)
  {
   CRCArduinoFastServos::writeMicroseconds(SERVO_RUD,Back);
   }
